<template>
  <h1 class="message"> {{ isRegistered ? 'Enregistrement' : 'Connexion' }} </h1>
  <v-card class="mx-auto px-6 py-8 mt-4" max-width="344">
    <v-form v-model="form" @submit.prevent="isRegistered ? onRegister() : onLogin()">
      <v-text-field v-model="mail" :readonly="loading" :rules="[required]" class="mb-2" label="E-mail" type="email"
        clearable />

      <v-text-field v-model="password" :readonly="loading" :rules="[required]" label="Mot de passe" type="password"
        placeholder="Entrer le mot de passe" clearable />
    ...
    <div v-if="msgConnexionError !== null">
      <p class="message mt-4" style="color: red">
        {{ msgConnexionError }}
      </p>
    </div>
  </v-card>
...
const isRegistered = props.register;
