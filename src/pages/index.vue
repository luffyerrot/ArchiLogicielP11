    ...
    <v-container>
        <v-row>
            <v-col :cols="msgSpecialite !== null ? 6 : 12">
                <v-card
                    class="mx-auto px-6 py-8 mt-4"
                    max-width="800"
                >
                    <h2 class="message ma-4">Choisir ma catégorie :</h2>
                    <v-list
                        v-for="(item, index) in msgGroupeSpecialite"
                        :key="'groupe' + index"
                    >
                        <v-list-item
                            :title="item.nom"
                            style="text-align: center"
                            @click="findSpecialites(item.id)"
                        />
                    </v-list>
                </v-card>
            </v-col>
            <v-col
                v-if="msgSpecialite !== null"
                cols="6"
            >
                <v-card
                    class="mx-auto px-6 py-8 mt-4"
                ...
                >
                    ...
                    <v-list
                        v-for="(item, index) in msgSpecialite"
                        :key="'specialite' + index"
                    >
                        <v-list-item
                            :title="item.nom"
                            style="text-align: center"
                            @click="findHopitaux(item.id)"
                        />
                    </v-list>
                </v-card>
            </v-col>
            <v-col
                v-if="msgHopital !== null"
                cols="12"
            >
                <v-card
                    class="mx-auto px-6 py-8 mt-4"
                ...
                >
                    ...
                    <v-list
                        v-for="(item, index) in msgHopital"
                        :key="'hopital' + index"
                    >
                        <v-list-item
                            :title="item.name + ' - ' + item.available_beds + ' lit⸱s restant⸱s'"
                            :subtitle="item.adresse + ' : ' + item.latitude + ' - ' + item.longitude"
                            style="text-align: center"
                            @click="toMapFinder(item.latitude, item.longitude)"
                        />
                    </v-list>
                </v-card>
            </v-col>
